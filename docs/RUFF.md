...
# 特定のファイル/ディレクトリ（直接実行）
uv run python generate_review_doc.py src/
```

> **注意:** このレポート生成はローカル開発用です。
> GitHub ActionsではreviewdogによるPRコメントのみが使用されます。

### レポートの内容

生成される `RUFF_REVIEW.md` には以下が含まれます：
...
`generate_review_doc.py` を編集して、レポートフォーマットを変更できます。
